...
<style>
  ...
  <form action="{{url('p/kelas',$detail->soal_master_id)}}/tambah-soal" method="POST">
     @csrf
    <input type="hidden" value="{{$detail->soal_mapel_id}}" name="soal_mapel_id">
    <input type="hidden" value="{{$detail->soal_kelas_nomor}}" name="soal_kelas_nomor">
    ...
    <a href="{{url('p/kelas',$detail->soal_mapel_id)}}/detail" onclick="return confirm('Tinggalkan form')" class="btn btn-dark">Selesai</a>
  </form>
  </div>
  </div>
  </div>
